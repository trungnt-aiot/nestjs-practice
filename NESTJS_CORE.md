...
- Task: `@Injectable()` is decorator used to injecting from service to controller. Without it, we cannot using `dependency injection`
...
- Task: Remember to install `class-validator` and `class-transformer` packages for validation to work
...
- Take task module for example, `task-create.dto.ts` file used to verify body of request:
...
- `task-update.dto.ts` file, the optional param:

```ts
import { PartialType } from '@nestjs/mapped-types';
import { NoteCreateDto } from './task-create.dto';
...
export class NoteDto {
  ...
  static fromEntity(task: Task): NoteDto {
    const noteDto = new NoteDto();
    noteDto.id = task.id;
    noteDto.tittle = task.tittle;
    noteDto.content = task.content;
    noteDto.userId = task.user.id;
    noteDto.username = task.user.username;
...
import { TypeOrmModule } from '@nestjs/typeorm';
import { UserModule } from './modules/user/user.module';
import { NoteModule } from './modules/task/task.module';
...
```ts
import { NoteCreateDto } from './dto/task-create.dto';
...
export class NoteProcessor extends WorkerHost {
  constructor(
    ...
    private readonly noteRepository: Repository<Task>,
  ...
  override async process(job: Job): Promise<void> {
    ...
    try {
      if (job.name === 'after_create_note') {
        ...
        const savedNote: Task = await this.noteRepository.save(newNote);

        this.logger.log(
          `[NoteProcessor] Created task ${savedNote.id} for user ${userId}`,
        );
      ...
      if (job.name === 'delete_note') {
        ...
        this.logger.log(`[NoteProcessor] Deleting task ${noteId}`);

        const task: Task | null = await this.noteRepository.findOne({
          where: { id: noteId },
          relations: ['user'],
        });

        if (!task) {
          this.logger.warn(`[NoteProcessor] NOT FOUND task ${noteId}`);
...
```bash
[2025-07-02T03:00:40.703Z] GET /task - ::1
[2025-07-02T03:00:56.284Z] DELETE /task/831e30b7-fe9c-435d-a93a-a15f4bd89800 - ::1
[2025-07-02T03:01:34.127Z] DELETE /task/831e30b7-fe9c-435d-a93a-a15f4bd89800 - ::1
[2025-07-02T03:02:28.800Z] GET /task/831e30b7-fe9c-435d-a93a-a15f4bd89800 - ::1
[2025-07-02T03:03:17.153Z] GET /task - ::1
[2025-07-02T03:04:29.311Z] POST /task - ::1
[2025-07-02T03:04:54.345Z] POST /task - ::1
[2025-07-02T03:05:11.179Z] POST /auth/refresh - ::1
[2025-07-02T03:05:17.382Z] POST /task - ::1
[2025-07-02T03:05:52.582Z] GET /task - ::1
[2025-07-02T03:26:55.958Z] GET /task - ::1
[2025-07-02T03:26:56.293Z] GET /task - ::1
[2025-07-02T03:26:56.715Z] GET /task - ::1
[2025-07-02T03:26:57.215Z] GET /task - ::1
[2025-07-02T03:26:58.375Z] GET /task - ::1
[2025-07-02T03:26:59.760Z] GET /task - ::1
[2025-07-02T03:27:00.292Z] GET /task - ::1
[2025-07-02T03:27:00.747Z] GET /task - ::1
[2025-07-02T04:33:54.410Z] GET / - ::1
[2025-07-02T04:33:56.723Z] GET /task - ::1
...
{"level":"info","message":"[NoteProcessor] Created task 7476bbbb-c41d-4eff-9cf0-b96fac19c1a2 for user 9bfe9367-a08f-4238-aabc-0b361bdb9117","timestamp":"10:05:17 2/7/2025"}
...
- Take `task.service.ts` file for example:
...
import { Task } from './task.entity';
import { InjectRepository } from '@nestjs/typeorm';
import { NoteDto } from './dto/task.dto';
import { NoteCreateDto } from './dto/task-create.dto';
import { NoteUpdateDto } from './dto/task-update.dto';
...
export class NoteService {
  constructor(
    @InjectRepository(Task)
    private readonly noteRepository: Repository<Task>,
    @InjectQueue('task') private noteQueue: Queue,
    @Inject(WINSTON_MODULE_NEST_PROVIDER)
    private readonly logger: LoggerService,
  ) {}
  ...
  async getAllNote(): Promise<NoteDto[]> {
    ...
    try {
      const notesList: Task[] = await this.noteRepository.find({
        relations: ['user'],
      });

      const result = notesList.map((task) => NoteDto.fromEntity(task));
      const duration = Date.now() - start;
      this.logger.log(
  ...
  async getNote(noteId: string): Promise<NoteDto | null> {
    ...
    try {
      const task = await this.noteRepository.findOne({
        where: { id: noteId },
        relations: ['user'],
      });
      ...
      const result = NoteDto.fromEntity(task);
      const duration = Date.now() - start;
  ...
  async update(noteUpdateDto: NoteUpdateDto, noteId: string): Promise<NoteDto> {
    ...
    try {
      ...
      return NoteDto.fromEntity(task);
    } catch (err) {
      const duration = Date.now() - start;
      this.logger.error(
...
export class AdvancedTaskService {
  ...
  async handleDatabaseBackup() {
    ...
    while (attempt < maxRetries) {
      ...
      this.customLogger.log(
        `[DatabaseBackup] ATTEMPT ${attempt}/${maxRetries}`,
      );
